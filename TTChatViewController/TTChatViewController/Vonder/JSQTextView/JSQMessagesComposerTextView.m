...
@interface JSQMessagesComposerTextView ()<UITextViewDelegate>
...
- (void)layoutSubviews
{
    ...
    if(self.heightConstraint.constant != newHeight){
        self.heightChangeBlock? self.heightChangeBlock(newHeight):nil;
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    self.textShouldBeginEditing? self.textShouldBeginEditing():nil;
    return YES;
}

- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text{
    if ([text isEqualToString:@"\n"]){
        self.textShouldReturnBlock? self.textShouldReturnBlock(self,self.text):nil;
        return NO;
    }
    return YES;
}
